...
cts <- read_data("data/verse_counts.tsv")
...
filtered <- filter_zero_var_genes(cts)
...
meta_info <- meta_info_from_labels(colnames(filtered)[-1])
...
sizes <- get_library_size(filtered)

ggplot(sizes, aes(x = sample, y = value, fill = sample)) +
  geom_bar(stat = "identity") +
  labs(title = "Library Sizes", y = "# reads", x = "Sample", fill = "Sample")

#Make a barplot showing the number of millions of reads for each sample

ggplot(sizes, aes(x = sample, y = value / 1e6, fill = sample)) +
  geom_bar(stat = "identity") +
  labs(title = "Library Sizes", y = "# reads", x = "Sample", fill = "Sample")
...
normalized_cpm <- normalize_by_cpm(filtered)
head(normalized_cpm)

write.csv(normalized_cpm, "normalized_counts.csv")
...
normalized_deseq <- deseq_normalize(filtered, meta_info)

head(normalized_deseq)
```
...
filtered_sample_distributions <- plot_sample_distributions(filtered, scale_y_axis = TRUE, "Sample Distributions (Log10(Raw Counts))")

filtered_sample_distributions
```
...
filtered_variance_vs_mean <- plot_variance_vs_mean(filtered, scale_y_axis = TRUE, title = "Variance vs. Mean (Log10(Raw Counts))")

filtered_variance_vs_mean
```
...
filtered_pca <- plot_pca(filtered, meta_info, "Raw Counts PCA")
...
cpm_sample_distributions <- plot_sample_distributions(normalized_cpm, scale_y_axis = TRUE, "Sample Distributions (Log10(CPM))")
cpm_variance_vs_mean <- plot_variance_vs_mean(normalized_cpm, scale_y_axis = TRUE, title = "Variance vs. Mean (Log10(CPM))")
cpm_pca <- plot_pca(normalized_cpm, meta_info, "CPM PCA")

cpm_sample_distributions
cpm_variance_vs_mean
cpm_pca
...
deseq_sample_distributions <- plot_sample_distributions(normalized_deseq, scale_y_axis = TRUE, "Sample Distributions (Log10(deseq Counts))")
deseq_variance_vs_mean <- plot_variance_vs_mean(normalized_deseq, scale_y_axis = TRUE, title = "Variance vs. Mean (Log10(deseq Counts))")
deseq_pca <- plot_pca(normalized_deseq, meta_info, "deseq Counts PCA")

deseq_sample_distributions
deseq_variance_vs_mean
deseq_pca
```
